...
from env_search.emitters import EvolutionStrategyEmitter, MapElitesBaselineWarehouseEmitter, RandomEmitter, IsoLineEmitter, PyCMAEmitter
...
class Manager:  # pylint: disable = too-many-instance-attributes
    ...
    Main args:
        ...
        env_manager_class: This class calls a separate manager based on the
            environment, such as WarehouseManager. Pass this class using this
            argument.
    ...
    Algorithm args:
        ...
        bounds: bounds of the solution space. Default to None.
    ...
    def __init__(
        ...
        # archive should store. For highway optimization, the solution size
        # is inferred from the number of valid edges in the given map.
        self.sol_size = self.env_manager.get_sol_size()

        # Process bounds and initial mean of the solutions
        self.bounds = [bounds for _ in range(self.sol_size)]
        ...
        if not reload:
            ...
            metric_list = [
                ...
                ("Total Num of Restarts", True),
                ("Num Restarts from Area", True),
            ]
            ...
            if self.is_em:
                ...
                # TODO: Initialize result archive properly while implementing
                # CMA-MAE version of DSAGE
                self.result_archive: ArchiveBase = None
    ...
    def add_performance_metrics(self):
        ...
        if self.is_cma_mae:
            total_restart = np.sum(
                [emitter.restarts for emitter in self.scheduler.emitters])
            total_restart_from_area = np.sum([
                emitter._n_restart_from_area
                for emitter in self.scheduler.emitters
            ])
        else:
            total_restart = 0
            total_restart_from_area = 0
        self.metrics.add(
            "Total Num of Restarts",
            total_restart,
            logger,
        )
        self.metrics.add(
            "Num Restarts from Area",
    ...
    def build_emitters_and_scheduler(self, archive):
        ...
        # We should start from a positive value because we are dealing with
        # edge costs
        initial_solution = np.empty(self.sol_size)
        initial_solution[:] = self.initial_mean
        ...
        self.result_archive = None
        if self.is_cma_mae:
            self.result_archive = self.archive_type(
                solution_dim=self.sol_size,
                learning_rate=1.0,
    ...
    def build_emulation_archive(self) -> ArchiveBase:
        ...
        archive: ArchiveBase = self.archive_type(
            solution_dim=self.sol_size,
            seed=seed,
            dtype=np.float32,
        ...
        if self.downsample_em:
            ...
            scales = np.array(archive.dims) // np.array(
            ...
            for downsample_idx in itertools.product(
                ...
                for idx in itertools.product(*archive_ranges):
                    ...
                    idx = archive.grid_to_int_index(
                        np.array(idx).reshape(1, 2))
                    if archive._occupied_arr[idx]:
                        elites.append(
                            Elite(archive._solution_arr[idx],
                                  archive._objective_arr[idx],
                                  archive._measures_arr[idx], idx,
                                  archive._metadata_arr[idx]))
    ...
    def evaluate_solutions(self, sols, parent_sols=None):
        ...
        for sol, r in zip(sols, results):
            if not r.failed:
                ...
                if self.is_em:
                    # Use obj_result for ground-truth archive in DSAME
        ...
        if self.num_elites_to_eval is None:
            ...
            for i, elite in enumerate(emulation_archive):
                ...
                if self.random_sample_em:
                    ...
                    new_elite = int(
                        self.archive.elite_with_behavior(elite.measures).obj is
                        None)
