...
class planner
{
    ...
    std::vector<std::vector<float>> dyn_costs;
    ...
    void compute_shortest_path()
    {
        ...
        while(!OPEN.empty() and !(current == goal))
        {
            ...
            for(auto n: get_neighbors({current.i, current.j})) {
                ...
                if(use_dynamic_cost)
                    cost += dyn_costs[n.first][n.second];
...
public:
    ...
    grid(_grid), use_static_cost(_use_static_cost), use_dynamic_cost(_use_dynamic_cost), reset_dynamic_cost(_reset_dynamic_cost)
    {
        ...
        dyn_costs = std::vector<std::vector<float>>(grid.size(), std::vector<float>(grid.front().size(), 0));
    ...
    std::vector<std::vector<float>> get_dyn_costs_matrix()
    {
        return dyn_costs;
    }
    ...
    void set_dyn_costs(std::vector<std::vector<float>> _dyn_costs)
    {
        dyn_costs = std::move(_dyn_costs);
    }
