...
def run_follower(env, net_params=None, model_path=None):
    if model_path is not None:
        follower_cfg = FollowerInferenceConfig(path_to_weights=model_path)
    else:
        follower_cfg = FollowerInferenceConfig()
    # follower_cfg.device = 'cpu'
    algo = FollowerInference(follower_cfg)

    follower_cfg.training_config.preprocessing.net_params = net_params
