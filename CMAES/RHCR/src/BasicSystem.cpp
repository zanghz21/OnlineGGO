...
void BasicSystem::update_paths(const std::vector<Path> &MAPF_paths, int max_timestep = INT_MAX)
{
    for (int k = 0; k < num_of_drives; k++)
    {
        ...
        for (int t = 0; t < length; t++)
        {
            ...
            // Small trick of manufacture system:
...
void BasicSystem::update_initial_paths(vector<Path> &initial_paths) const
{
    ...
    for (int k = 0; k < num_of_drives; k++)
    {
        ...
        {
            while (j >= 0 &&
                   paths[k][j].location != std::get<0>(goal_locations[k][i]) &&
                   paths[k][j].timestep >= std::get<1>(goal_locations[k][i]))
                {
                    j--;
                }
            i--;
...
list<tuple<int, int, int>> BasicSystem::move()
{
    ...
    list<tuple<int, int, int>> new_finished_tasks; // <agent_id, location, timestep>
    ...
    for (int t = start_timestep; t <= end_timestep; t++)
    {
        for (int k = 0; k < num_of_drives; k++)
        {
            ...
            // Update the tasking wait. Used only by manufacture system.
            if (curr.is_tasking_wait)
            {
                this->is_tasking[k] = true;
                this->waited_time[k] += 1;
                if (this->screen > 0)
                {
                    cout << "Agent " << k << " doing task at " << curr.location
            ...
            // DONE:
            // 1. add another field in goal_location to remember the number
            //    of timesteps the agent needs to wait at the current goal
            // 2. add a new varaible to remember the number of tiemstep the
            //    agents have waited at the current timestep
            // 2.1 easier but slower: in paths, calculate number of
            //     timesteps the agents has waited on the fly.
            if ((!hold_endpoints || paths[k].size() == t + 1) && !goal_locations[k].empty() &&
                curr.location == std::get<0>(goal_locations[k].front()) &&
                // the agent arrived at the goal
                curr.timestep >= std::get<1>(goal_locations[k].front()) &&
                // the agent finished its task at the goal
                this->waited_time[k] >= std::get<2>(goal_locations[k].front()))
            {
                goal_locations[k].erase(goal_locations[k].begin());
                new_finished_tasks.emplace_back(k, curr.location, t);
...
void BasicSystem::solve()
{
    ...
    else // PBS or ECBS
    {
        ...
        if (hold_endpoints || useDummyPaths)
        {
            ...
            vector<vector<tuple<int, int, int>>> new_goal_locations;
        ...
        else
        {
            ...
            vector<vector<tuple<int, int, int>>> real_goal_locations;
            real_goal_locations.resize(num_of_drives);
            for (int k = 0; k < num_of_drives; k++)
            {
                int j = 0;
                if (starts[k].is_tasking_wait)
                {
                    j = 1;
                }
