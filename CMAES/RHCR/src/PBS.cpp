    ...
    const vector<int>& waited_time)
{
    ...
    // TODO: add `is_manufacture` param syn for other algos
    path_planner.is_manufacture = is_manufacture;
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        ...
        {
            min_sum_of_costs += G.heuristics.at(std::get<0>(goal))[start_loc];
            start_loc = std::get<0>(goal);
        }
    }
	if (screen > 0) // 1 or 2
		print_results();
...
void PBS::print_paths() const
{
	for (int i = 0; i < num_of_agents; i++)
	{
        ...
        {
            cout << s.location;
            if (s.is_tasking_wait) cout << "(wait)";
            cout << "->";
        }
		std::cout << std::endl;
	}
