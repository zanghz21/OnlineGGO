...
Path SIPP::updatePath(const BasicGraph& G, SIPPNode* goal)
{
    ...
    while (true)
    {
        ...
        if(curr->tasking_wait > 0)
        {
            ...
            {
                path[t] = State(
                    curr->state.location, t, curr->state.orientation, true);
            }
               ...
               const int agent_waited_time)
{
	...
	if (hold_endpoints)
		earliest_holding_time = rt.getHoldingTimeFromSIT(
    ...
    while (!focal_list.empty())
    {
        ...
        if (!this->is_manufacture)
        {
            // update goal id
            if (curr->state.location == std::get<0>(curr_goal) &&
                // reach the goal location after its release time
                curr->state.timestep >= std::get<1>(curr_goal))
            {
                curr->goal_id++;
            ...
            while (curr->state.location == std::get<0>(curr_goal) &&
                // reach the goal location after its release time
			   curr->state.timestep >= std::get<1>(curr_goal) &&
               curr->goal_id < (int)goal_location.size())
            {
                ...
                // Increase timestep
                curr->state.timestep += std::get<2>(curr_goal);
                curr->tasking_wait += std::get<2>(curr_goal);
