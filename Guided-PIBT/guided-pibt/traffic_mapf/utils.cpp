...
int get_id_from_d(int d, int curr_id, const SharedEnvironment* env){
    int diff = (d==0)? 1: (d==1)? env->cols: (d==2)? -1: -(env->cols);
    int next_id = curr_id + diff;
    if (next_id < 0 || next_id >= env->map.size()){
        next_id = -1;
    }
    return next_id;
}
...
void getNeighborLocs(const SharedEnvironment* env, std::vector<int>& neighbors, int location) {
	...
	//forward
	assert(location >= 0 && location < env->map.size());
    neighbors = env->neighbors[location];
    return;
...
void getNeighborLocs(const SharedEnvironment* env, int neighbors[], int location) {
	...
	for (int i = 0; i < size; i++) {
		if (i < env->neighbors[location].size()){
			neighbors[i] = env->neighbors[location][i];
		}
        else
            neighbors[i] = -1;
	}
