...
bool is_swap_required(int pusher, int puller, 
	int pusher_loc, int puller_loc, 
    std::vector<int>& prev_decision, std::vector<int>& traffic,TrajLNS& lns){
	int tmp_loc = -1;

	while (get_heuristic(lns.heuristics[lns.tasks.at(pusher)], lns.env, traffic, lns.flow, puller_loc)
	< get_heuristic(lns.heuristics[lns.tasks.at(pusher)], lns.env, traffic, lns.flow, pusher_loc)){
		std::vector<int> neighbors;
		getNeighborLocs(lns.env,neighbors,puller_loc);
		int n = neighbors.size();
		for (auto& neighbor: neighbors){
			int a = prev_decision.at(neighbor);
			if (neighbor == pusher_loc 
			// || (lns.env->neighbors[neighbor].size() == 1 && a!=-1 
			// 	&& lns.env->goal_locations[a][0].first == neighbor)
			){
				--n;
			} else{
				tmp_loc = neighbor;
		...
		if (n >= 1) return false;
		if (n <= 0) break;
		pusher_loc = puller_loc;
		puller_loc = tmp_loc;
	}

	return (get_heuristic(lns.heuristics[lns.tasks.at(puller)], lns.env, traffic, lns.flow, pusher_loc) 
	< get_heuristic(lns.heuristics[lns.tasks.at(puller)], lns.env, traffic, lns.flow, puller_loc))
	&&
	(get_heuristic(lns.heuristics[lns.tasks.at(pusher)], lns.env, traffic, lns.flow, puller_loc) 
	< get_heuristic(lns.heuristics[lns.tasks.at(pusher)], lns.env, traffic, lns.flow, pusher_loc));
}

bool is_swap_possible(int pusher_loc_origin, int puller_loc_origin, 
    std::vector<int>& prev_decision, std::vector<int>& traffic,TrajLNS& lns){
	int puller_loc = puller_loc_origin;
	int pusher_loc = pusher_loc_origin;
	int tmp_loc = -1;

	while (puller_loc != pusher_loc_origin){
		std::vector<int> neighbors;
		getNeighborLocs(lns.env,neighbors,puller_loc);
		int n = neighbors.size();
		for (auto& neighbor: neighbors){
...
int swap_possible_and_required(int ai, std::vector<State>& successors, 
	std::vector<State>& prev_states, std::vector<State>& next_states,
    std::vector<int>& prev_decision, std::vector<int>& decision, 
	std::vector<int>& traffic,TrajLNS& lns){
	
	int prev_loc = prev_states[ai].location;
	if (successors[0].location == prev_loc){
		return -1;
	}
	int aj = prev_decision.at(successors[0].location);
	if (aj != -1 && next_states[aj].location == -1 &&
		is_swap_required(ai, aj, prev_states[ai].location, prev_states[aj].location, 
		prev_decision, traffic, lns) &&
		is_swap_possible(prev_states[aj].location, prev_states[ai].location, 
		prev_decision, traffic, lns)){
	...
	for (auto& neighbor: neighbors){
		int ak = prev_decision.at(neighbor);
		if (ak == -1 || successors[0].location == prev_states[ak].location) continue;
		if (is_swap_required(ak, ai, prev_states[ai].location, successors[0].location, 
		prev_decision, traffic, lns) 
		&& is_swap_possible(successors[0].location, prev_states[ai].location, 
		prev_decision, traffic, lns)){
...
bool causalPIBT(int curr_id, int higher_id,std::vector<State>& prev_states,
...
#ifdef SWAP
	int swap_agent = swap_possible_and_required(curr_id, successors,  
    ...
    for (auto& next: successors){
		...
		// avoid edge conflicts caused by swap action
		int ak = prev_decision.at(next.location);
		if (ak != -1 && next_states.at(ak) == prev_states.at(curr_id)){
			continue;
		}
		...
		#ifdef SWAP
		if (cnt == 0 && swap_agent != -1 && next_states.at(swap_agent).location == -1
			&& decision.at(prev_states.at(curr_id).location) == -1
		){
			next_states.at(swap_agent) = prev_states.at(curr_id);
			decision.at(next_states.at(swap_agent).location);
		}
		#endif
